void updateMask() {
  ...
  if (keyPressed && key == ' ') { 
    kinectDepthBackgroundImage.copy(kinect.getDepthImage(), 0, 0, kinectDepthImage.width, kinectDepthImage.height, 0, 0, kinectDepthBackgroundImage.width, kinectDepthBackgroundImage.height); 
    println("copying background image");  
  }
  ...
  println(blobs.length + " blobs detected");
  scaleBlobs();
...
void scaleBlobs() {
  if (blobs != null) {
    for (int i = 0; i < blobs.length; i++) {
      ...
      blobs[i].rectangle = new Rectangle(r.x * 2, r.y * 2, r.width * 2, r.height * 2);
    }
  }
}
...
void drawBlobs(Blob[] blobs, PGraphics pg) {
  if (blobs != null) {
    for( int i = 0; i < blobs.length; i++) {
      ...
      dir.normalize();
      dir.mult(distance * 0.75);
      dir.add(startPoint);
      pg.stroke(255, 0, 0);
      pg.line(startPoint.x, startPoint.y, dir.x, dir.y);
    }
  }
}
